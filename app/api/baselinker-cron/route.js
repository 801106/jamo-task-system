...
export async function GET(req) {
  ...
  try {
    // Sync last 1 day (cron runs every 6h so 1 day is safe overlap)
    ...
    if (data.status !== 'SUCCESS') {
      console.error('BaseLinker cron error:', data.error_message)
    ...
    for (const order of orders) {
      ...
      if (existing) {
        ...
        await supabase.from('client_interactions').insert({
          ...
          content: `[Auto] Zamowienie #${order.order_id} — ${marketplace} — £${orderValue.toFixed(2)}`,
        })
        updated++
      } else {
        ...
        if (newClient) {
          await supabase.from('client_interactions').insert({
            ...
            content: `[Auto] Pierwsze zamowienie #${order.order_id} — ${marketplace} — £${orderValue.toFixed(2)}`,
          })
          created++
        }
      }
    ...
    console.log(`BaseLinker cron: ${orders.length} orders, ${created} created, ${updated} updated`)
    return Response.json({ success: true, orders_processed: orders.length, clients_created: created, clients_updated: updated, skipped })

  } catch (error) {
    console.error('BaseLinker cron exception:', error)
